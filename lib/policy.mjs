...
export const ADVISE_ALLOW = [
  ...
  // NOTE: `sed` is deliberately NOT here. Even `sed -n` admits `-i` (in-place
  // write) and the `e` command / `s///e` flag (shell exec); a regex cannot
  // reliably fence sed's script language. Use head/tail for line ranges.
  /^\s*git\s+status(\s|$)/,
  /^\s*git\s+diff(\s|$)/,
  /^\s*git\s+log(\s|$)/,
]
...
// NOTE: `sort` and `uniq` are deliberately excluded — `sort -o FILE` and
// `uniq INPUT OUTPUT` both write a file from an apparent filter position.
export const SAFE_FILTER = [
  /^\s*(tail|head|wc|cat|grep|rg|cut|tr)(\s|$)/,
]
...
function adviseAllowsShell(command) {
  ...
  // The shell removes quotes and backslashes before it runs the command, so a
  // danger flag can hide as `--pre""`, `-"delete"`, or `-delet\e` and slip past
  // a regex that inspects the raw string. Strip them first and match on what
  // the shell will actually execute. Legitimate quoted reads (`rg "a b" .`)
  // survive \u2014 the head still matches after the quotes are gone.
  ...
  const seqs = bare.split('&&').map(s => s.trim()).filter(Boolean)
