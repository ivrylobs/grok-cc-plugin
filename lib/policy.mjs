...
// read-only filters. Rejects shell machinery and weaponized flags, then checks
// head + downstream filters.
function segmentAllowed(seg, heads) {
  if (SHELL_MACHINERY.test(seg)) return false           // bare & / ; / quotes / subst / brace / redirect
  if (ADVISE_DANGER_FLAG.test(seg)) return false        // plain --pre, --output, find -delete/-exec, …
...
function adviseAllowsShell(command) {
  ...
  // Split on `&&` BEFORE segmentAllowed's bare-`&` test (0001): every segment
