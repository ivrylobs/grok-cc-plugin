...
function partIsReadOnly(part, allowTests) {
...
function segmentAllowed(seg, allowTests) {
  if (SHELL_MACHINERY.test(seg)) return false           // bare & / ; / quotes / subst / brace / redirect
  const parts = seg.split('|').map(s => s.trim()).filter(Boolean)
  return parts.length > 0 && parts.every(p => partIsReadOnly(p, allowTests))
}
...
function adviseAllowsShell(command) {
  ...
  const allowTests = process.env.GROK_CC_ADVISE_TESTS === '1'
  ...
  return seqs.every(s => segmentAllowed(s, allowTests))
}
...
export function decideToolCall(grip, toolCall) {
  ...
  if (grip === 'read') {
    ...
    // via the same whitelist as advise. Writes/edits are NEVER auto-allowed — and
    // decideToolCall can only return allow|ask, so this is only half the fence:
    // fs-mediator.writeTextFile HARD-REFUSES under grip 'read' (a captain 'allow'
    // to a write ask still cannot land a byte). We return 'ask' so the attempt is
    // at least surfaced, not silently allowed.
    if (toolCall?.kind === 'read') return 'allow'
    if (toolCall?.kind === 'execute' && adviseAllowsShell(command)) return 'allow'
    return 'ask'
  }
