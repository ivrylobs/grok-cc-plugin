/**
 * CALIBRATION FOIL for Problem P2 (ZonedRecurrence).
 * HIDDEN — scoring instrument only. Never enters any arm's context.
 *
 * Models a strong-but-rushed one-session solo solve. Four deliberate degradations,
 * each a documented real-world library failure mode:
 *  (1) wall->instant via "guess offset, refine once" — no gap/overlap candidate
 *      enumeration; right on unique times, wrong on some gaps;
 *  (2) count applied AFTER exDate removal (RFC-folklore refill);
 *  (3) WEEKLY weeks anchored on start's date, not Monday-based calendar weeks;
 *  (4) positive byMonthDay clamps to month end.
...
/**
 * DEGRADATION (1): "guess offset, refine once". Correct whenever the wall time
 * maps uniquely; silently wrong on some gaps/overlaps depending on the zone's
 * offset sign and transition hour.
 */
function resolveWall(w: LocalDateTime, zone: string): { ms: number; wall: LocalDateTime } {
  const naive = naiveMs(w);
  const off1 = offsetAt(naive, zone);
  const off2 = offsetAt(naive - off1, zone);
  const ms = naive - off2;
...
function* candidateDays(v: Validated): Generator<number> {
  ...
  if (v.freq === "DAILY") {
    ...
    // DEGRADATION (3): 7-day blocks anchored on start's DATE, not Monday-based weeks.
    if (v.byDay) {
      const wds = new Set(v.byDay.map((t) => t.wd));
      for (let b = startDay; ; b += 7 * v.interval) {
        for (let d = b; d < b + 7; d++) if (wds.has(weekdayMon0(d))) yield d;
      }
    } else {
      for (let d = startDay; ; d += 7 * v.interval) yield d;
    }
    ...
    for (let k = 0; ; k++) {
      ...
      if (v.byMonthDay) {
        for (const md of v.byMonthDay) {
          // DEGRADATION (4): positive monthdays clamp to the month's last day.
          const d = md > 0 ? Math.min(md, dim) : dim + 1 + md;
...
export function expandBetween(spec: RecurrenceSpec, fromUtc: string, toUtc: string): Occurrence[] {
  ...
  for (const ed of candidateDays(v)) {
    ...
    if (v.untilMs !== undefined && ms > v.untilMs) continue;
    // DEGRADATION (2): excluded candidates do NOT consume count (series refills).
    if (v.exDates.some((x) => wallEq(x, wall))) continue;
    produced++;
