...
import { managedGrokHome } from './grok-home.mjs'
...
export class AcpClient {
  constructor({ cwd, onUpdate = () => {}, onAgentRequest = null }) {
    // route every tool call through our permission gate (see grok-home.mjs)
    const home = managedGrokHome()
    const env = home ? { ...process.env, GROK_HOME: home } : process.env
    this.child = spawn(GROK_BIN, ['agent', 'stdio'], { cwd, stdio: ['pipe', 'pipe', 'ignore'], env })
    // A write racing the child's exit emits an async EPIPE 'error' on the pipe;
    // unhandled, it crashes the process. Absorb it — `closed` handles the exit.
  ...
  async _onLine(line) {
    ...
    if (msg.id !== undefined && msg.method === undefined) {              // reply to our request
      ...
      try {
        ...
        // JSON-RPC error codes must be integers; Node fs errors carry string codes ('ENOENT')
        const code = Number.isInteger(e.code) ? e.code : -32603
        this._send({ jsonrpc: '2.0', id: msg.id, error: { code, message: `${e.code ?? ''} ${e.message}`.trim() } })
      }
    }
