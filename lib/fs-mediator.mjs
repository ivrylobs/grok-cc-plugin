...
export function makeFsHandlers(meta) {
  ...
  return {
    ...
    async writeTextFile({ path: p, content }) {
      // read grip is the enforcement point, not the policy: decideToolCall can
      // only return allow|ask, so a captain who answers 'allow' to a write ask
      // must still not land a byte. Refuse here — and do NOT stage, staging is a
      // side effect too. readTextFile is untouched.
      if (meta.grip === 'read') {
        audit(meta.id, { op: 'write-refused', path: p, grip: 'read' })
        const err = new Error('read grip: file writes are refused')
        err.code = 'READ_GRIP_REFUSED'
        throw err
      }
      const abs = containOrDeny(p)
