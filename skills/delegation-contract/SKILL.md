...
- Second opinion / audit / co-thinker that must NOT touch the tree -> `--grip read` (reads + read-only shell only; mediated writes are hard-refused, not staged). Untrusted or production tree -> `--grip gate` (writes staged until approve-stage). Trusted mechanical -> `--grip leash`. Default `advise`. `read` is a subprocess boundary, not a sandbox: under `read`, `git diff`/`git log -p` auto-allow only with `--no-textconv --no-ext-diff` (they run the repo's configured textconv/ext-diff — arbitrary code — by default); never point `read` at a `.git/config` you didn't write.
