...
Grips, least to most trusted write access: `read` (no mediated writes at all — reads + read-only shell only, for a second opinion or audit that must not touch the tree), `gate` (writes stage until you `approve-stage`), `advise` (in-tree writes auto-allowed, risky shell asks), `leash` (auto-allow except a deny-list — trusted mechanical work only). `read` is a subprocess boundary, not a sandbox — it means "no mediated file writes + no free-form shell," not "no subprocess touches disk": an allow-listed `git status` still refreshes `.git/index`. Sharpest caveat: `git diff`/`git log -p` run the repo's own configured `textconv`/`ext-diff` driver (arbitrary code) **by default**, so under `read` they auto-allow only with `--no-textconv --no-ext-diff`; under `advise`/`leash` (trusted-tree grips) bare `git diff` still auto-allows, which trusts that tree's `.git/config`. Never aim `read` at a directory whose `.git/config` you didn't write.
